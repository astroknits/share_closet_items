...
def parse_args():
    parser = ArgumentParser(
        description=('Help file for share_war.py'
            'from the poshmark_sharing repository:'
            'https://github.com/jmausolf/poshmark_sharing'),
        usage='use "python %(prog)s --help" for more information')
    parser.add_argument("-t", "--time", default=7200, type=int,
        help=('loop time in seconds to repeat the code (e.g. -t 7200 for two hours)'))
    ...
    parser.add_argument("-n", "--num_items", default=0, type=int,
        help="select a random subset (number) of items to share (default=0 (share all))")
